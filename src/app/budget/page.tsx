...
export default function Budget() {
  const { transactions, budgets, setBudget, deleteBudget, profile } = useStore();

  const period = getBudgetPeriod(profile.budgetResetDay ?? 5);

  const breakdown = useMemo(() => {
    const m: Record<string, { amount: number; count: number; percentage: number }> = {};
    const periodTxs = transactions.filter((t) => {
      const d = t.date.slice(0, 10);
      return t.type === 'expense' && d >= period.start && d <= period.end;
    });
    const total = periodTxs.reduce((s, t) => s + t.amount, 0);
    periodTxs.forEach((t) => {
      if (!m[t.category]) m[t.category] = { amount: 0, count: 0, percentage: 0 };
      m[t.category].amount += t.amount;
      m[t.category].count++;
    });
    Object.keys(m).forEach((k) => { m[k].percentage = total > 0 ? (m[k].amount / total) * 100 : 0; });
    return m;
    ...
    <Layout>
      ...
      {showAdd && (
        <div className="mx-4 mb-3 card p-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-3">
            {categories.map((cat) => (
              <button key={cat.id} onClick={() => setSelCat(cat.id)}
                ...
                style={selCat === cat.id ? { backgroundColor: cat.color + '22', borderColor: cat.color } : { backgroundColor: '#181818', borderColor: '#2A2A2A' }}>
          ...
          <input className="w-full bg-surface2 border border-border rounded-xl px-3 py-3 text-white text-xl font-bold placeholder-muted mb-3"
            placeholder="Monto límite" value={budgetAmt}
            onChange={(e) => setBudgetAmt(e.target.value)} inputMode="numeric" autoFocus />
      ...
      <div className="px-4">
        {monthBudgets.length === 0 && !showAdd ? (
          <div className="flex flex-col items-center py-16 px-6">
            ...
            <p className="text-muted text-sm text-center mb-6">Define cuánto quieres gastar por categoría en este período.</p>
            <button onClick={() => setShowAdd(true)} className="btn-primary text-sm px-6">Crear primer presupuesto</button>
          </div>
        ) : (
          <>
            {monthBudgets.map((b) => (
              <BudgetBar key={b.categoryId} categoryId={b.categoryId}
                budget={b.amount} spent={breakdown[b.categoryId]?.amount || 0}
            ...
            {Object.entries(breakdown)
              ...
              .map(([catId, data]) => (
                <button key={catId} onClick={() => { setSelCat(catId); setShowAdd(true); }}
